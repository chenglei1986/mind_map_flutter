...
void main() {
  group('Tags, Icons, and Images Property Tests', () {
    test(
      'Tags and icons rendering - for any node with tags or icons, the rendering output should contain all tags and icons',
      () {
    ...
    test(
      'Property 6: Image size correctness - for any node containing an image, the rendered image size should match the specified width and height',
      () {
        const iterations = 100;
    ...
    test(
      'Property: Combined tags, icons, and images rendering - for any node with all three, all elements should be accounted for in size calculation',
      () {
        const iterations = 100;
...
NodeData _generateNodeWithImage(int seed) {
  ...
  return NodeData.create(topic: 'Node with image $seed', image: image);
}
...
NodeData _generateNodeWithAllElements(int seed) {
  ...
  final tags = List.generate(tagCount, (i) => TagData(text: 'Tag${i + 1}'));
