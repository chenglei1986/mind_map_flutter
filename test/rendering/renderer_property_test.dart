...
void main() {
  group('Renderer Property Tests', () {
    ...
    test(
      'Property 3: Branch connection integrity - for any node with parent-child relationship, there should be a branch line connecting them with color from theme palette',
      () {
    ...
    test(
      'Property 4: Style application consistency - for any node with custom style, the rendering output should contain all specified style properties',
      () {
        const iterations = 100;
...
NodeData _generateNodeWithCustomStyle() {
  ...
  return NodeData.create(topic: 'Styled Node $random', style: style);
}
